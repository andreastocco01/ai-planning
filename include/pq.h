...
class PriorityQueue {
   public:
    ...
    /** Changes the score of an integer @param j already in the queue to @param
     * p */
    ...
    bool checkConsistency() const {
        ...
        for (int k = 0; k < cnt; k++) {
            int left = 2 * k + 1;
            int right = 2 * k + 2;
            if ((left < cnt) && (prior[data[left]] < prior[data[k]]))
                return false;
            if ((right < cnt) && (prior[data[right]] < prior[data[k]]))
                return false;
        }
        // check positions
        for (int k = 0; k < cnt; k++) {
    ...
    void print() const {
        for (int k = 0; k < cnt; k++) {
            std::cout << "\t" << k << "\t" << data[k] << "\t" << prior[data[k]]
                      << std::endl;
        }
    }
   ...
   protected:
    ...
    /**
     * Moves the gap up from the current position @param gap to the proper place
     * for a priority of value @param p The final position of the gap is stored
     * in @param gap
     */
