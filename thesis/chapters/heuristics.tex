...
\begin{figure}[ht]
	...
	\caption{Behaviour of the $h^{\max}$ heuristic without pruning}
...
\begin{figure}[ht]
	...
	\caption{Behaviour of the $h^{\max}$ heuristic with pruning}
...
Here, $h_{sp}\left(p;s_*\right)$ denotes the estimated cost of achieving the fact \textit{p} from the goal state $s_*$.
