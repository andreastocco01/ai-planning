...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State \Call{hmax}{currentState, goal}
		\EndFor
		\State $possibleActions \gets \{a \in A \mid pre(a) \subseteq currentState\}$
		\If {$\Call{empty}{possibleActions}$}
		\State \textbf{return} $failure$
...
\verb|A*| selects actions based on the sum of these two values, leading to optimal solutions when an admissible heuristic (like \verb|HMAX|) is used.
